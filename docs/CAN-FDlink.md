...
CAN FDlink 规定了**命令帧、配置帧、数据帧、心跳帧**4种帧类型。目前，命令帧和配置帧的传输采用“**请求-响应**“方式，数据帧和心跳帧“**周期性定时触发**”方式，后续数据帧考虑加入SYNC同步触发方式。

CAN FDlink 采用**一主多从**架构，逻辑上一个CAN FDlink只能有一个主节点（主站）。
...
CAN FDlink 协议最大提供 8 位站址，地址“0”是广播地址不允许使用，节点可以使用的地址范围1~127，128~255 地址保留。主节点默认为最末的127是为了和Modbus从机地址兼容。

| 节点     | 地址范围                 |
| -------- | ------------------------ |
| 广播地址 | 0                        |
| 节点地址 | 1-127（主节点默认为127） |
| 保留地址 | 128-255                  |
...
| ------ | ----------------------------------------- |
| 命令帧 | 不定，具体由命令编码决定                  |
| 配置帧 | [Modbus CANFD](Modbus CANFD.md) 的PDU部分 |
| 数据帧 | 数据                                      |
| 心跳帧 | 无                                        |
...
| 帧类型 | 数据结构方式                                     |
| ------ | ------------------------------------------------ |
| 配置帧 | 为兼容Modbus协议，16位、32位数据均采用大端模式   |
| 数据帧 | 考虑到数据访问效率，16位、32位数据均采用小端模式 |

- 配置帧中，32位数据需要占用两个连续的寄存器，此时也是高寄存器在前、低寄存器在后，并且每个寄存器内部依然是高字节在前
