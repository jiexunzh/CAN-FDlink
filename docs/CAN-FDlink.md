...
由于CAN FDlink兼容Modbus协议，采用16位寄存器 高字节在前的表示方式。16位、32位数据均采用**大端模式**。32位数据需要占用两个连续的16位寄存器，此时是低寄存器存放高16位，高寄存器存放低16位，并且每个寄存器内部依然是高字节在前，例如，0x12345678 对应的REG_L：0x12 0x34，REG_H：0x56 0x78。
...
命令编码“0xFF”固定为命令异常响应帧的命令编码，表示从节点执行或解析某个主节点发来的命令帧出现异常。命令异常响应帧的数据是异常码，长度为1字节。
...
- bit24：问答标识，区分请求与响应，命令“1”，响应“0”
- bit23-16：帧序号，用于后续拓展单次访问最大寄存器数量时区别不同的报文，目前固定为0
- bit15-8：目标节点，接收该帧的节点地址
- bit7~0：源节点，产生该帧的节点地址

数据部分：

- 有效数据长度（2字节）：Modbus PDU的有效负载长度（虽然Modbus PDU本身具有长度感知，理论上只要知道是请求帧or应答帧和功能码，即可解析出Modbus报文，但加上PDU长度可以让解析更可靠、更快，也有利于未来拓展寄存器数量或自定义功能码）
...
CAN FDlink 数据帧用于CAN FDlink 设备之间的实时数据传输与共享。主节点通过**配置帧**配置后从节点心跳时间、上报数据索引、上报时间间隔等参数后，发送启动**命令帧**，从节点收到并应答后，开始按配置规则周期性的向主节点发送**数据帧**。

数据帧格式如下：

| 节点 | bit28~25      | bit24      | bit23~16 | bit15~8                 | bit7~0                | 数据                          |
| ---- | ------------- | ---------- | -------- | ----------------------- | --------------------- | ----------------------------- |
| 从   | 帧标识：1100B | 帧类型标识 | 帧序号   | 目标节点：yyH（主节点） | 源节点：xxH（从节点） | 有效数据长度 + 数据 (+空数据) |

拓展帧ID部分：

- bit28-25：帧标识，固定为"1100b"，以区分不同帧类型
- bit24：数据帧类型标识
  - “0”表示点对点数据帧（发给一个主站）
  - “1”表示点对多数据帧（发给多个主站）
- bit23-16：帧序号，用于后续拓展单次最大发送数据量时区别不同的报文（目前固定为0）
- bit15-8：目标节点，接收该帧的节点地址
- bit7~0：源节点，产生该帧的节点地址

数据部分：

- 有效数据长度（2字节）：有效数据部分的长度（目前最多能传输的数据量为62字节）。

- 数据：根据“用户自定义通信对象映射表”的配置，传输用户希望传输的通信对象。
- 空数据：当PDU长度大于8且不符合DLC时，需要在PDU后**补0**，补至最接近的DLC；补的0值不参与PDU解析，也不改变有效数据长度

## 6. CAN FDlink 心跳帧(0xE)

CAN FDlink心跳帧是由节点周期性发出的心跳信号，目前主要用于主节点监测各个从节点状态。主节点应事先配置好从节点的心跳时间，比如1000ms。心跳帧的数据是节点状态，长度为1字节。

从节点心跳帧结构如下：

| 节点 | bit28~25      | bit24 | bit23~16      | bit15~8  | bit7~0 | 数据     |
| ---- | ------------- | ----- | ------------- | -------- | ------ | -------- |
| 从   | 帧标识：1110B | 保留  | 心跳编码：02H | 节点地址 | 保留   | 节点状态 |

ID部分：

- bit28-25：帧标识，固定为"1110b"，以区分不同帧类型
- bit24：保留，默认为0；
- bit23~16：心跳编码，02H表示从节点心跳；
- bit15~8：节点地址，目前主要是指从节点地址
- bit7~0：保留
