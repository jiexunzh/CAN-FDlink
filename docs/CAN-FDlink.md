...
CAN FDlink 规定了**命令帧、配置帧、数据帧、心跳帧**4种帧类型。命令帧和配置帧的传输采用“**请求-响应**“方式，数据帧和心跳帧“**周期性定时触发**”方式，后续数据帧考虑加入SYNC同步触发方式。

CAN FDlink 目前采用**一主多从**架构，逻辑上一个CAN FDlink只能有一个主节点（主站）。
...
注：配置帧中，32位数据需要占用两个连续的寄存器，此时也是高寄存器在前、低寄存器在后，并且每个寄存器内部依然是高字节在前。比如存放：0x12345678，对应的REG1：0x78 0x56，REG2：0x34 0x12。
...
- bit24：问答标识，区分请求与响应，命令发送“1”，响应“0”
...
- 不定：需要根据具体命令编码的约定作解析，数据长度在0~64字节之间
...
命令编码“0xFF”固定为命令异常响应帧的命令编码，表示从节点执行或解析某个主节点发来的命令帧出现异常。命令异常响应帧结构如下所示：

| 节点 | bit28~25      | bit24 | bit23~16       | bit15~8                 | bit7~0                | 数据   |
| ---- | ------------- | ----- | -------------- | ----------------------- | --------------------- | ------ |
| 从   | 帧标识：0100b | 答：0 | 命令编码：0xFF | 目标节点：xxH（主节点） | 源节点：yyH（从节点） | 异常码 |
...
CAN FDlink 配置帧采用“请求-响应“方式，用于主节点访问和配置指定从节点寄存器。CAN FDlink 配置帧的数据承载的是Modbus协议的PDU部分，寄存器地址和数据同Modbus协议都是16位宽。目前，CAN FDlink配置帧**不依赖上下文**，可独立解析，单次最多可读**31**个寄存器，单次最多可写**29**个寄存器。除广播命令外，需要从节点应答。配置帧的具体内容可参见 [Modbus over CANFD](Modbus over CANFD.md) 

配置帧结构如下所示：

| 节点 | bit28~25      | bit24 | bit23~16 | bit15~8     | bit7~0  | 数据                   |
| ---- | ------------- | ----- | -------- | ----------- | ------- | ---------------------- |
| 主   | 帧标识：1010b | 问：1 | 保留     | 从节点：yyH | PDU长度 | Modbus PDU（+ 空数据） |
| 从   | 帧标识：1010b | 答：0 | 保留     | 从节点：yyH | PDU长度 | Modbus PDU（+ 空数据） |

拓展帧ID部分：

- bit28-25：帧标识，固定为"1000b"，以区分不同帧类型
- bit24：问答标识
- bit23-16：保留，默认为0；后续如需拓展单次读写寄存器最大数量，可定义为包序号
- bit15-8：从节点地址，地址范围为1-126（主节点地址默认127），**建议与Modbus从站地址保持一致**
- bit7-0：PDU长度；虽然Modbus PDU本身具有长度感知，理论上只要知道是请求帧or应答帧和功能码，即可解析出Modbus报文；但加上PDU长度可以让解析更可靠、更快，也有利于未来拓展寄存器数量或自定义功能码

数据部分：

- Modbus PDU：采用标准的Modbus PDU，包括：功能码 + 数据
- 空数据：当Modbus PDU长度大于8且不符合DLC时，在PDU后**补0**至最接近的DLC

## 5. CAN FDlink 数据帧(0xC)

CAN FDlink 数据帧用于CAN FDlink 设备之间的快速数据传输。主节点通过配置帧对从节点进行配置后，发送启动命令帧；收到启动命令的从节点应答命令，然后按配置规则周期性定时、主动向主节点发送数据帧。

数据帧格式如下：

| 节点 | bit28~25      | bit24 | bit23~16 | bit15~8     | bit7~0 | 数据 |
| ---- | ------------- | ----- | -------- | ----------- | ------ | ---- |
| 从   | 帧标识：1100B | 保留  | 命令编码 | 从节点：yyH | 包序号 | 不定 |
...
| 节点 | bit28~25      | bit24 | bit23~16      | bit15~8     | bit7~0 | 数据       |
| ---- | ------------- | ----- | ------------- | ----------- | ------ | ---------- |
| 从   | 帧标识：1110B | 保留  | 心跳编码：02H | 从节点：yyH | 保留   | 从节点状态 |
