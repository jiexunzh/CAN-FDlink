...
由于CAN FDlink兼容Modbus协议，采用16位寄存器的高字节在前的表示方式。16位、32位数据均采用**大端模式**。32位数据需要占用两个连续的寄存器，此时也是低寄存器在前、高寄存器在后，并且每个寄存器内部依然是高字节在前。例如，32位数据0x12345678，对应的REG_L：0x78 0x56，REG_H：0x34 0x12。
