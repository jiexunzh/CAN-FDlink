...
CAN FDlink 是按多主从进行设计的，但目前实际应用最多的是**一主多从**的场景。
...
| 数据帧 | 考虑到数据访问效率，16位、32位数据均采用小端模式，其他类型帧的数据默认也是小端模式 |

配置帧中，32位数据需要占用两个连续的寄存器，此时也是高寄存器在前、低寄存器在后，并且每个寄存器内部依然是高字节在前。比如存放：0x12345678，对应的REG1：0x78 0x56，REG2：0x34 0x12。
...
| 节点 | bit28~25      | bit24 | bit23~16 | bit15~8                 | bit7~0                | 数据                    |
| ---- | ------------- | ----- | -------- | ----------------------- | --------------------- | ----------------------- |
| 主   | 帧标识：1010b | 问：1 | 帧序号   | 目标节点：xxH（从节点） | 源节点：yyH（主节点） | PDU长度 + PDU (+空数据) |
| 从   | 帧标识：1010b | 答：0 | 帧序号   | 目标节点：yyH（主节点） | 源节点：xxH（从节点） | PDU长度 + PDU (+空数据) |
...
- PDU长度：Modbus PDU的有效负载长度（虽然Modbus PDU本身具有长度感知，理论上只要知道是请求帧or应答帧和功能码，即可解析出Modbus报文，但加上PDU长度可以让解析更可靠、更快，也有利于未来拓展寄存器数量或自定义功能码）

- PDU：采用标准的Modbus PDU，包括：功能码 + 数据
- 空数据：当PDU长度大于8且不符合DLC时，需要在PDU后**补0**，补至最接近的DLC；补的0值不参与PDU解析，也不改变PDU长度

## 5. CAN FDlink 数据帧(0xC)

CAN FDlink 数据帧用于CAN FDlink 设备之间的快速数据传输。主节点通过配置帧对从节点上报间隔等参数进行配置，发送启动命令帧；从节点收到启动命令后，发送应答命令，然后开始按配置规则周期性的向主节点发送数据帧。
